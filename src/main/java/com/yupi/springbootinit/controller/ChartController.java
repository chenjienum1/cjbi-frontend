...
public class ChartController {
    ...
    public BaseResponse<BiResponse> genChartByAi(@RequestPart("file") MultipartFile multipartFile,
        ...
        ThrowUtils.throwIf(!list.contains(suffix),ErrorCode.PARAMS_ERROR,"文件后缀错误");
        User loginUser = userService.getLoginUser(request);
        //限流判断，每个用户一个限流器
        redissonLimitManager.doRateLimit("genChartByAi_"+loginUser.getId());


        StringBuffer sb=new StringBuffer();
        //sb.append("你现在是一个数据分析师，现在我要给你分析目标个数据，请告诉我分析结论").append("\n");
        sb.append("分析目标："+goal).append("\n");
        if(StringUtils.isNotBlank(chartType)){
            sb.append(",请使用").append(chartType);
        }
        long cur2 = System.currentTimeMillis();
        String csvData = ExcelUtils.excelToCsv(multipartFile);
        String res = sb.append("原始数据：").append(csvData).append("\n").toString();
        long modelId=1674659615717011458L;
        String result = aiManager.doChart(res, modelId);
        ...
        ThrowUtils.throwIf(!save,ErrorCode.SYSTEM_ERROR,"库表保存错误");

        BiResponse response=new BiResponse();
        response.setGenChart(genChart);
        response.setGenResult(genResult);

        log.info("准备时间：{}",cur2-cur1);
        log.info("发送时间：{}",cur3-cur2);
    ...
    public BaseResponse<BiResponse> genChartByAiAsync(@RequestPart("file") MultipartFile multipartFile,
        ...
        CompletableFuture.runAsync(()->{
            ...
            runChart.setStatus("running");
            boolean b = chartService.updateById(runChart);
           if(!b){
               System.out.println("更新为执行错误");
               handleChartUpdateError(chart.getId(),"更新为执行中错误");
               return;
           }
            ...
            succeedChart.setChartData(csvData);
            succeedChart.setGenChart(genChart);
            succeedChart.setGenResult(genResult);

            //插入数据到数据库
            boolean succeedUpdate=chartService.updateById(succeedChart);
            if(!succeedUpdate){
                System.out.println("更新失败");
                handleChartUpdateError(chart.getId(),"更新为成功错误");
                return;
            }
